# Subtitle Translator — Combined Documentation

This single document merges installation, usage, GUI layout, examples, optimization guidance, performance summary, and quick references into one place.
...
1. Install Python 3.11+ (use your platform's package manager or download from python.org).
...
Common CLI flags (examples from consolidated docs):
...
- Formatting tags (ASS tags like `{1}`, `{\i1}`, `{\pos(x,y)}`) are protected by placeholders during translation and restored afterward.
- Line breaks: `\N` and `\n` are preserved.
